<?php
    ...
    if (isset($_REQUEST['submit'])) {
        ...
        $query1 = "INSERT INTO `users` (username, password, email, access_level, trn_date)
        VALUES ('$username', '".md5($password)."', '$email', '$access_level', '$trn_date')";

        // Execute the first query
        $result1 = mysqli_query($conn, $query1);

        if ($result1) {
            // Retrieve the auto-generated user ID from the first query
            $id = mysqli_insert_id($conn);

            // Insert data into 'registration' table
            $query2 = "INSERT INTO `registration` (id_login, register_as, title, full_name, phone_number, address, research_field, university)
            VALUES ('$id', '$register_as', '$title', '$full_name', '$phone_number', '$address', '$research_field', '$university')";
